...
async def _update_order_status(
    ...
    """Helper function to update order status with inventory management."""
    tenant_id = get_tenant_id(x_tenant_id)
    ...
    with get_db_cursor() as cur:
        ...
        # Get order line items for inventory operations
        cur.execute("""
            SELECT soi.id, soi.product_id, soi.quantity_ordered
            FROM sales_order_items soi
            WHERE soi.sales_order_id = %s AND soi.product_id IS NOT NULL
        """, (str(order_id),))
        ...
        if warehouse_id and line_items:
            # CONFIRMED: Reserve inventory
            if new_status == SalesOrderStatus.CONFIRMED:
                reservation_errors = []
                for item in line_items:
                    cur.execute("""
                        SELECT * FROM reserve_inventory(
                            %s, %s, %s, %s, 'sales_order', %s, %s
                        )
                    """, (
                        str(tenant_id),
                        str(item['product_id']),
                    ...
                    if not result['success']:
                        reservation_errors.append(f"Product {item['product_id']}: {result['message']}")
                
                if reservation_errors:
                    raise HTTPException(
                        status_code=400,
                        detail=f"Inventory reservation failed: {'; '.join(reservation_errors)}"
                    )
            ...
            # CANCELLED: Release reservations
                ...
                for item in line_items:
                    cur.execute("""
                        ...
                        AND status = 'active'
                    """, (str(order_id), str(item['id'])))
                    reservation = cur.fetchone()
                    if reservation:
                        cur.execute("""
                            SELECT * FROM release_reservation(%s, 'cancel')
                        """, (str(reservation['id']),))
            
            # SHIPPED: Fulfill reservations (deduct from on_hand)
            elif new_status == SalesOrderStatus.SHIPPED:
                for item in line_items:
                    cur.execute("""
                        SELECT id FROM inventory_reservations
                        WHERE source_type = 'sales_order' 
                    ...
                    if reservation:
                        cur.execute("""
                            SELECT * FROM release_reservation(%s, 'fulfill')
                        """, (str(reservation['id']),))
                    
                    # Update line item status
                    cur.execute("""
                        UPDATE sales_order_items 
                        SET status = 'shipped', quantity_shipped = quantity_ordered, updated_at = CURRENT_TIMESTAMP
                        WHERE id = %s
                    """, (str(item['id']),))
    ...
    with get_db_cursor(dict_cursor=True) as cur:
        try:
            ...
            platform_map = {
                ...
                "swiggy": Platform.INSTAMART,
                "instamart": Platform.INSTAMART,
            }
            platform = platform_map.get(payload.platform.lower(), Platform.MANUAL)
            ...
            for idx, item in enumerate(payload.items, 1):
                ...
                unit_price = item.unit_price or (float(product['selling_price']) if product and product.get('selling_price') else 0)
                line_total = item.quantity * unit_price
                
                cur.execute("""
                    INSERT INTO sales_order_items (
                        sales_order_id, product_id, sku, product_name,
                        quantity_ordered, unit_price, line_total, status,
                ...
                """, (
                    ...
                    unit_price,
                    line_total,
                    LineItemStatus.CONFIRMED.value,  # Already allocated by AMS
                    idx
                ))
            
            # Note: Totals are auto-calculated by database trigger (trg_recalculate_so_totals)
            # No manual update needed
            
            # Log status history
            cur.execute("""
                INSERT INTO sales_order_status_history (
                    sales_order_id, new_status, notes, changed_by_name
                )
                VALUES (%s, %s, %s, %s)
            """, (
                str(order_id),
