...
async def preview_import(
    ...
    # Match products with catalog using multiple identifiers
    with get_db_cursor() as cur:
        for order in orders_preview:
            for item in order["line_items"]:
                ...
                # 1. Try to find by EAN (external_id)
                if item.get("external_id"):
                    cur.execute("""
                        SELECT p.id, p.name, p.sku, 'ean' as matched_by
                        FROM product_identifiers pi
                        JOIN products p ON pi.product_id = p.id
                        WHERE pi.tenant_id = %s
                        AND pi.identifier_type = 'ean'
                        AND pi.identifier_value = %s
                        LIMIT 1
                    """, (str(tenant_id), item["external_id"]))
                    product = cur.fetchone()
                
                # 2. Try to find by ASIN
                if not product and item.get("asin"):
                    cur.execute("""
                        SELECT p.id, p.name, p.sku, 'asin' as matched_by
                        FROM product_identifiers pi
                        JOIN products p ON pi.product_id = p.id
                        WHERE pi.tenant_id = %s
                        AND pi.identifier_type = 'asin'
                        AND pi.identifier_value = %s
                        LIMIT 1
                    """, (str(tenant_id), item["asin"]))
                    product = cur.fetchone()
                
                # 3. Try to find by old SKU alias
                if not product and item.get("sku"):
                    cur.execute("""
                        SELECT p.id, p.name, p.sku, 'sku_alias' as matched_by
                        FROM product_identifiers pi
                        JOIN products p ON pi.product_id = p.id
                        WHERE pi.tenant_id = %s
                        AND pi.identifier_type = 'sku_alias'
                        AND pi.identifier_value = %s
                        LIMIT 1
                    """, (str(tenant_id), item["sku"]))
                    product = cur.fetchone()
                
                # 4. Fallback: Try direct SKU match
                if not product and item.get("sku"):
                    cur.execute("""
                        SELECT id, name, sku, 'sku' as matched_by
                        FROM products
                        WHERE tenant_id = %s
                        AND sku = %s
                        LIMIT 1
                ...
                if product:
                    ...
                    item["matched_by"] = product.get("matched_by", "unknown")
                else:
                    order["validation_messages"].append(
                        f"Product not found: EAN={item.get('external_id')}, ASIN={item.get('asin')}, SKU={item.get('sku')}"
                    )
            ...
            try:
                ...
                if fc_code:
                    ...
                    if not fc:
                        ...
                        cur.execute("""
                            INSERT INTO fulfillment_centers (tenant_id, code, name, full_name, platform)
                            VALUES (%s, %s, %s, %s, %s)
                            RETURNING id
                        """, (
                            str(tenant_id),
                ...
                cur.execute("""
                    INSERT INTO sales_orders (
                        ...
                        expected_shipment_date, delivery_window_start, delivery_window_end,
                        status, priority
                ...
                """, (
                    ...
                    order_data.get("fulfillment_center_full", fc_code),
                    expected_shipment,
                    expected_shipment,  # delivery_window_start
                    None,  # delivery_window_end (parse later if needed)
                    "pending_confirmation",  # Imported orders start as pending confirmation
                    "normal"
                ))
                ...
                for item in order_data.get("line_items", []):
                    ...
                    cur.execute("""
                        INSERT INTO sales_order_items (
                            sales_order_id, product_id, sku, product_name,
                            asin, external_id,
                    ...
                    """, (
                        ...
                        item.get("product_id"),
                        item.get("sku", ""),
                        item.get("name", "Unknown Product")[:500],
                        item.get("asin", ""),
                ...
                cur.execute("""
                    UPDATE sales_orders
                    SET
                        subtotal = (SELECT COALESCE(SUM(quantity_ordered * unit_price), 0) FROM sales_order_items WHERE sales_order_id = %s),
                        total_amount = (SELECT COALESCE(SUM(line_total), 0) FROM sales_order_items WHERE sales_order_id = %s),
                        total_items = (SELECT COUNT(*) FROM sales_order_items WHERE sales_order_id = %s),
                        total_quantity = (SELECT COALESCE(SUM(quantity_ordered), 0) FROM sales_order_items WHERE sales_order_id = %s),
                        updated_at = CURRENT_TIMESTAMP
                    WHERE id = %s
                """, (str(order_id), str(order_id), str(order_id), str(order_id), str(order_id)))
